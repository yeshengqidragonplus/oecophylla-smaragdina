...
每个实例同时使用 UDP 和 TCP 两类通道，职责严格分离：
...
```
 ...
 广播 ◄──────── │    · 仅收 discovery_request（同机多实例都能收到广播）           │
                │                                                              │
 可靠流         │  TCP 服务（主端口 + 1000，TcpTransport）                       │
 ◄────────────► │    · 所有业务消息：聊天文本、文件分块                           │
                │                                                              │
                └──────────────────────────────────────────────────────────────┘
```
...
- **TCP 负责"说什么"**：聊天消息和文件全部走 TCP。可靠、有序、自带流控，文件分块无需自己实现重传。
- 身份标识 **peerId = `ip:主UDP端口`**。TCP 端口不直接参与身份，通过 UDP 消息的 `transferPort` 字段告知对端（默认为主端口 + 1000）。

> 历史决策：早期版本使用 kcpjs（KCP over UDP）承载业务消息，但 kcpjs 依赖原生模块（需要 C++ 工具链编译），且内网场景下 KCP 相对 TCP 无实际优势，已整体替换为 Node 内置 `net`（TCP）。

### 2.2 模块与数据流

```
                        ┌────────────────┐
   网络事件（EventEmitter）│ networkService │◄──── TcpTransport（由它创建并持有）
                        └───────┬────────┘
...
| `src/tcpTransport.ts` | TCP 业务通道：监听、连接、帧编解码、身份识别、会话表（peerId → socket） |
...
所有线上消息（UDP 与 TCP）共用 `NetworkMessage` 接口，JSON 编码，`type` 字段区分：
...
| `message` | TCP | 聊天文本（`content` 字段） |
| `file` | TCP | 文件分块（`file.{name,size,chunkIndex,totalChunks,data(base64)}`） |
...
- TCP 会话成功建立（`sessionConnected`）同样会把离线 peer 拉回在线——TCP 建连本身就是在线证明。

### 3.3 TCP 帧与身份协议

- **帧格式**：4 字节大端长度前缀 + JSON 负载。接收侧按前缀拆帧，解决 TCP 流式拆包；单帧上限 64 MB，超限直接断连防内存攻击。
- **身份帧**：连接建立后第一帧必须是 `{"__id": "<发起方peerId>"}`。接收方据此把 socket 注册到 peerId，之后的帧才作为业务数据上报。无效身份帧 → 关闭连接。
- **双向同时建连仲裁**：A、B 可能同时向对方发起连接，产生两条冗余链路。约定 **peerId 字典序较小的一方保留其出站连接**，另一条销毁。双方独立执行此规则得到一致结果，无需协商。
- **背压**：`send()` 在 `socket.write()` 返回 false 时等待 `drain`，避免大文件把发送缓冲撑爆。
- **在线检测即建连**：发送前不做额外 UDP 握手，TCP 建连（5 秒超时）失败即视为对方不在线。
...
2. `executeTransferTask()`：先确保 TCP 会话，然后顺序发送文本消息，再逐个文件分块发送。
...
| 配置 | 默认 | 说明 |
|---|---|---|
| `port` | 8080 | UDP 主端口；TCP 业务端口 = 此值 + 1000 |
...
- 重建 TcpTransport 前必须先 `removeAllListeners()` + `stop()`（历史上有事件监听泄漏 bug）。
- `NetworkService.start()` 失败时回滚已启动的部分（TCP 起来了但 UDP 失败 → 先停 TCP 再抛错）。
- `stop()` 清理顺序：定时器 → TcpTransport（销毁所有 socket）→ UDP 主 socket → 发现 socket，最后将所有 peer 置离线并广播 `peersUpdate`。

## 8. 安全考量与已知限制

- **无加密无认证**：协议明文 JSON，身份帧可伪造。设计前提是受信任的小型内网；不要在不可信网络使用。
- 接收文件名已做路径穿越防护（§4），这是唯一的输入硬化点之一；TCP 帧长度上限防止恶意超大帧。
